...
#ifndef BU_LOG_CRYPT_H_
#define BU_LOG_CRYPT_H_
...
class BULogCrypt {
public:
    BULogCrypt(const char* _pubkey);
    virtual ~BULogCrypt() {}
    
private:
    BULogCrypt(const BULogCrypt&);
    BULogCrypt& operator=(const BULogCrypt&);
...
public:
    ...
    bool CryptSyncLog(const char* const _log_data, size_t _input_len, BUAutoBuffer& _out_buff);
    bool CryptAsyncLog(const char* const _log_data, size_t _input_len, BUAutoBuffer& _out_buff, size_t& _remain_nocrypt_len);
